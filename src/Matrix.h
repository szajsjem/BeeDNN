...
public:
  ...
  static const Matrix<T> from_raw_buffer(const T *pData, Index iRows,
                                         Index iColumns) {
    Matrix<T> m;
  ...
  Index rows() const { return _iRows; }

  Index cols() const { return _iColumns; }

  Index size() const { return _iSize; }
  ...
  void setConstant(T b) { std::fill(_data, _data + _iSize, b); }
  ...
  void setOnes() { setConstant(1.); }
  void setOnes(Index iRows, Index iColumns) {
    resize(iRows, iColumns);
    setOnes();
  }

  void setRandom() { setRandomUniform(*this, -1.f, 1.f); }
  ...
  Matrix<T> operator+(T d) const { return Matrix<T>(*this).operator+=(d); }
  ...
  T norm() const { return ::sqrtf(squaredNorm()); }

  T mean() const { return sum() / (T)_iSize; }
  ...
  Matrix<T> middleRows(Index start, Index count) const {
    assert(start >= 0);
    assert(count >= 0);
    assert(start + count <= _iRows);
    return Matrix<T>(_data + start * _iColumns, count, _iColumns);
  }

  Matrix<T> middleCols(Index start, Index count) const {
    assert(start >= 0);
    assert(count >= 0);
    assert(start + count <= _iColumns);

    Matrix<T> out(_iRows, count);
    for (Index r = 0; r < _iRows; r++) {
      for (Index c = 0; c < count; c++) {
        out(r, c) = operator()(r, start + c);
      }
    }
    return out;
  }

  Matrix<T> col(Index iC) const {
    assert(iC < _iColumns);
    Matrix<T> r(_iRows, 1);
    for (Index i = 0; i < _iRows; i++)
      r(i, 0) = operator()(i, iC);
...
const MatrixFloatView viewResize(const MatrixFloat &m, Index iRows,
                                 Index iCols);
const MatrixFloatView
viewRow(const MatrixFloat &m, Index iStartRow,
        Index iEndRow); // create a row view starting at iStartRow to (not
                        // included) iEndRow
...
void setQuickBernoulli(MatrixFloat &m,
                       float fProba); // quick bernoulli is 6x faster than ref
                                      // bernoulli, resolution proba is 1/65536
...
void rowsArgmax(const MatrixFloat &m,
                MatrixFloat &argM); // compute the argmax row by row
