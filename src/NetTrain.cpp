...
/////////////////////////////////////////////////////////////////////////////////////////////////
NetTrain::NetTrain() : _sOptimizer("Adam"), _epochCallBack(nullptr) {
  _fTrainLoss = 0.f;
  _fTrainAccuracy = 0.f;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::clear() { clear_optimizers(); }
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
const Net &NetTrain::model() const { return *_pNet; }
/////////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::set_optimizer(
    const string
        &sOptimizer) //"Adam by default, ex "SGD" "Adam" "Nadam" "Nesterov"
{
  _sOptimizer = sOptimizer;
  clear_optimizers();
}
string NetTrain::get_optimizer() const { return _sOptimizer; }
/////////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::set_regularizer(
    const string &sRegularizer,
    float fParameter) // "" by default, can be also "Identity", "Clamp" ...
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::set_learningrate(
    float fLearningRate) //"Adam by default, ex "SGD" "Adam" "Nadam" "Nesterov"
{
...
float NetTrain::get_learningrate() const {
  if (!_optimizers.empty())
    return _optimizers[0]
        ->get_learningrate(); // get real lr if initialized with -1.f ;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
int NetTrain::get_epochs() const { return _iEpochs; }
/////////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::set_reboost_every_epochs(
    int iReboostEveryEpochs) //-1 by default -> disabled
{
  _iReboostEveryEpochs = iReboostEveryEpochs;
}
/////////////////////////////////////////////////////////////////////////////////////////////////
int NetTrain::get_reboost_every_epochs() const { return _iReboostEveryEpochs; }
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
string NetTrain::get_loss() const { return _pLoss->name(); }
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
Index NetTrain::get_batchsize() const { return _iBatchSize; }
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
bool NetTrain::get_RandomBatchOrder() const { return _bRandomBatchOrder; }
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
Index NetTrain::get_batchstepsize() const { return _iBatchStepSize; }
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
bool NetTrain::get_classbalancing() const { return _bClassBalancingWeightLoss; }
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
bool NetTrain::get_keepbest() const { return _bKeepBest; }
/////////////////////////////////////////////////////////////////////////////////////////////////
float NetTrain::compute_loss_accuracy(const MatrixFloat &mSamples,
                                      const MatrixFloat &mTruth,
                                      float *pfAccuracy) const {
...
/////////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::set_train_data(const MatrixFloat &mSamples,
                              const MatrixFloat &mTruth) {
  _pmSamplesTrain = &mSamples;
  _pmTruthTrain = &mTruth;
}
/////////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::set_train_data_copy(const MatrixFloat &mSamples,
                                   const MatrixFloat &mTruth) {
  _pmSamplesTrain = new MatrixFloat(mSamples);
  _pmTruthTrain = new MatrixFloat(mTruth);
}
/////////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::set_validation_data(const MatrixFloat &mSamplesValidation,
                                   const MatrixFloat &mTruthValidation) {
...
void NetTrain::fit(Net &rNet) {
  ...
  if (iNbSamples == 0)
    return; // nothing to do
  rNet.init((size_t)mSamples.cols());

  Net bestNet;

  if (_iBatchStepSize > 0)
    _iBatchStepAdjusted = _iBatchStepSize;
  else
    _iBatchStepAdjusted = _iBatchSize;
  ...
  if (_bKeepBest) {
    if (_pmSamplesValidation == nullptr) {
      fMinLoss = compute_loss_accuracy(*_pmSamplesTrain, *_pmTruthTrain,
                                       &fMaxAccuracy);
    } else {
      fMinLoss = compute_loss_accuracy(*_pmSamplesValidation,
                                       *_pmTruthValidation, &fMaxAccuracy);
    }
    bestNet = *_pNet;
  }
  ...
  for (int iEpoch = 0; iEpoch < _iEpochs; iEpoch++) {
    ...
    if (_pmSamplesValidation != nullptr) {
      ...
      _fValidationLoss = compute_loss_accuracy(
          *_pmSamplesValidation, *_pmTruthValidation, &_fValidationAccuracy);
    ...
    if (_bKeepBest || (_iPatience != -1)) {
      if (_pNet->is_classification_mode()) { // use accuracy
...
void NetTrain::slowfit(Net &rNet) {
  ...
  const MatrixFloat &mSamples = *_pmSamplesTrain;
  const MatrixFloat &mTruth = *_pmTruthTrain;
  const MatrixFloat &mSamplesV =
      _pmSamplesValidation != NULL ? *_pmSamplesValidation : *_pmSamplesTrain;
  const MatrixFloat &mTruthV =
      _pmTruthValidation != NULL ? *_pmTruthValidation : *_pmTruthTrain;
  ...
  trainableWeights.insert(trainableWeights.end(), _pWeights.begin(),
                          _pWeights.end());
...
/////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::train_batch(const MatrixFloat &mSample,
                           const MatrixFloat &mTruth) {
  assert(_pNet);

  for (auto &x : _gradient) { // clear gradients
    x.setZero();
  }
  ...
  for (int i = (int)_iNbLayers - 1; i >= 0; i--)
    _pNet->layer(i).backpropagation(
        _inOut[i], _gradient[(size_t)i + 1], _gradient[i],
        _pNet->internalCalculationMatrices(), _pNet->startICM(i));
...
/////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::add_online_statistics(const MatrixFloat &mPredicted,
                                     const MatrixFloat &mTruth) {
  // update loss
  MatrixFloat mLoss;
  ...
  if (mPredicted.cols() == 1) {
    ...
    if (mTruth.cols() == 1) {
      ...
      for (int i = 0; i < iNbRows; i++)
        _iOnlineAccuracyGood +=
            (argmax(mPredicted.row(i)) == argmax(mTruth.row(i)));
    }
  }
}
/////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////
float NetTrain::get_current_validation_loss() const { return _fValidationLoss; }
/////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////
float NetTrain::get_current_train_loss() const { return _fTrainLoss; }
/////////////////////////////////////////////////////////////////////////////////////////////
float NetTrain::get_current_train_accuracy() const { return _fTrainAccuracy; }
/////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////
void NetTrain::train_one_epoch(const MatrixFloat &mSampleShuffled,
                               const MatrixFloat &mTruthShuffled) {
