...
public:
  ...
  void set_train_data_copy(const MatrixFloat &mSamples,
                           const MatrixFloat &mTruth);
  void set_validation_data(const MatrixFloat &mSamplesValidation,
                           const MatrixFloat &mTruthValidation);

  void set_epochs(int iEpochs); // 100 by default
  int get_epochs() const;

  /// reboost helps the optimizer to have a new fresh start every N epochs,
  /// improve Adamax for example
  void
  set_reboost_every_epochs(int iReboostEveryEpochs); //-1 by default -> disabled
  int get_reboost_every_epochs() const;

  void set_epoch_callback(std::function<void()> epochCallBack);

  void set_optimizer(
      const std::string &sOptimizer); //"Adam by default, ex "SGD" "Adam"
                                      //"Nadam" "Nesterov" "iRPROP-" ...
  ...
  void
  set_learningrate(float fLearningRate = -1.f); // -1.f is for default settings
  float get_learningrate() const;

  void
  set_patience(int iPatience); // divide by two the learning rate if no progress
                               // during iPatience epochs , -1 is no patience
  ...
  void set_classbalancing(
      bool bBalancing); // true by default //use weight loss algorithm
  bool get_classbalancing() const;

  void
  set_keepbest(bool bKeepBest); // true by default: keep the best model of all
                                // epochs (evaluated on the test database)
  bool get_keepbest() const;

  void set_loss(
      const std::string &sLoss); // "MeanSquareError" by default, ex
                                 // "MeanSquareError" "CategoricalCrossEntropy"
  void set_loss(Loss *loss) { _pLoss = loss; }
  ...
  virtual void
  train_batch(const MatrixFloat &mSample,
              const MatrixFloat &mTruth); // all the backprop is here

protected:
  virtual void train_one_epoch(const MatrixFloat &mSampleShuffled,
                               const MatrixFloat &mTruthShuffled);
  void add_online_statistics(
      const MatrixFloat &mPredicted,
      const MatrixFloat &mTruth); // online statistics, i.e. loss, accuracy ..
...
private:
  ...
  void update_class_weight(); // compute balanced class weight loss (if asked)
                              // and update loss
