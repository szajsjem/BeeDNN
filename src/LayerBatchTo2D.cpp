...
namespace beednn {
LayerBatchTo2D::LayerBatchTo2D(const int incolsize, const int outcolsize,
...
LayerBatchTo2D::~LayerBatchTo2D() { delete _l2d; }

Layer *LayerBatchTo2D::clone() const {
  return new LayerBatchTo2D(_incolsize, _outcolsize, _outrowsize,
                            _l2d->clone());
}

bool LayerBatchTo2D::init(size_t &in, size_t &out, std::vector<MatrixFloat> &internalCalculationMatrices, bool debug) {
  if (in % _incolsize != 0)
    return false;
  out = _outcolsize * _outrowsize;
...
void LayerBatchTo2D::forward(const MatrixFloat &mIn, MatrixFloat &mOut) {
  ...
  for (int i = 0; i < mIn.rows(); i++) {
    MatrixFloat out;
    _l2d->forward(
        fromRawBuffer(mIn.row(i).data(), mIn.cols() / _incolsize, _incolsize),
        out);
...
void LayerBatchTo2D::backpropagation(const MatrixFloat &mIn, const MatrixFloat &mGradientOut, MatrixFloat &mGradientIn, std::vector<MatrixFloat> &internalCalculationMatrices, int start) {
  ...
  for (int i = 0; i < mIn.rows(); i++) {
    MatrixFloat gradin,
        ...
        gradOut =
            fromRawBuffer(mGradientOut.row(i).data(), _outrowsize, _outcolsize);
	_l2d->backpropagation(in, gradOut, gradin, internalCalculationMatrices, start);//todo check internalCalculationMatrices,start
    if (!_bFirstLayer)
      mLocalGrad.row(i) = fromRawBuffer(gradin.data(), 1, gradin.size());
    if (i == 0) {
...
bool LayerBatchTo2D::has_weights() const { return _l2d->has_weights(); }
...
///////////////////////////////////////////////////////////////
Layer *LayerBatchTo2D::load(std::istream &from) { return NULL; }
///////////////////////////////////////////////////////////////
Layer *LayerBatchTo2D::construct(std::initializer_list<float> fArgs,
                                 std::string sArg) {
  return NULL;
}
///////////////////////////////////////////////////////////////
std::string LayerBatchTo2D::constructUsage() {
  return "reshapes batch input to 2D "
