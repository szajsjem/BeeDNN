...
void set_weight(Layer &l, MatrixFloat &m) { *(l.weights()[0]) = m; }

MatrixFloat &get_gradient(Layer &l) { return *(l.gradient_weights()[0]); }
//////////////////////////////////////////////////////////////////////////////
void compare_im2col() {
  cout << "Comparing im2col() and im2col_LUT():" << endl;

  Index iNbSamples = 7, inRows = 31, inCols = 23, inChannels = 13,
        outChannels = 17; // all primes numbers
...
void compare_fastlut_slow_computation() {
  ...
  size_t ic = mIn.cols(), oc = -1;
  conv2d.init(ic, oc, internalCalculationMatrices);
  ...
  conv2d.fastLUT = false;
  conv2d.backpropagation(mIn, mOut, mIm, internalCalculationMatrices, 0);
  conv2d.fastLUT = true;
  conv2d.backpropagation(mIn, mOutFast, mImFast, internalCalculationMatrices,
                         0);
...
void forward_backward() {
  ...
  Index iNbSamples = 5, inRows = 7, inCols = 11, inChannels = 13,
        outChannels = 17; // all primes numbers
  MatrixFloat mIn, mCol, mColLUT, mIm, mImLUT;
  ...
  conv2d.backpropagation(mIn, mCol, mIm, internalCalculationMatrices, 0);
...
void forward_time() {
  ...
  LayerConvolution2D conv2d(iInRows, iInCols, iInChannels, iKernelRows,
                            iKernelCols, iOutChannels);
