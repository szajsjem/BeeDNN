...
int nextToken(std::string str, int &start, int end,
              std::vector<std::string> vocabulary) {
  for (int i = 0; i < vocabulary.size(); i++) {
    int c = strcmp(str.substr(start, vocabulary[i].size()).data(),
                   vocabulary[i].data());
    if (c == 0) {
...
std::vector<int> tokenizeString(std::string data,
                                std::vector<std::string> vocabulary) {
...
void testLayerGradientImpl(Layer *ln, MatrixFloat inputData) {
  ...
  // We strictly should init it, but for a standalone gradient test where init
  // might likely be skipped in this old test code... Assuming the layer doesn't
  // crash on backprop without init (or we init it if possible). The previous
  // code didn't Init. We pass 0 as start index.
  ln->backpropagation(inputData, mG, mB, internalCalculationMatrices, 0);
...
void test() {
  ...
  Layer *ln = new LayerTransformerHeads(
      4, 4, 4, 4, "Uniform", "Uniform"); // new LayerTransformerFeedForward(4,
                                         // 16, "Relu", "Uniform", "Uniform");//
  // new LayerNormalize();
...
char *rndstr(int l) {
  static const char *sl = "qwertyuiopasdfghjklzxcvbnm"; // QWERTYUIOPASDFGHJKLZXCVBNM";//,./<>?;':\"[]
                                                        // {}-=_+!@#$%^&*()~`\\|
...
int main() {
  ...
  Net model;
  model.add(
      new LayerEmbed(vocabSize, dimmSize, seqlen,
                     "Uniform")); // new LayerBatchTo2D(1, dimmSize, seqlen,
  for (int i = 0; i < layers; i++) {
    model.add(new LayerTransformerHeads(dimmSize, QKVmem, QKVmem, numheads,
                                        "Uniform", "Uniform"));
    model.add(
        new LayerTransformerFeedForward(dimmSize, FFmem, "Relu", "Uniform",
                                        "Uniform")); // modified version below
    /*model.add(new LayerParallel({
            new LayerActivation("Identity"),
            new LayerSequential({
                    new LayerNormalize(),
                    new LayerDense(dimmSize, FFmem * 2, "Uniform", "Uniform"),
                    new LayerGatedActivation("Tanh","Relu"),//added gated
       activation for tests new LayerDense(FFmem, dimmSize, "Uniform",
       "Uniform")})
            }, SUM));*/
  }
  model.add(
      new LayerNormalize()); // new LayerBatchTo2D(dimmSize, dimmSize, dimmSize,
  model.add(new LayerDense(
      dimmSize, vocabSize, "Uniform",
      "Uniform")); // new LayerBatchTo2D(dimmSize, vocabSize, vocabSize,
  model.add(new LayerSoftmax()); // new LayerBatchTo2D(vocabSize, vocabSize,
                                 // vocabSize,
  ...
  for (int i = 0; i < iSamples; i++) {
    for (int s = 0; s < 1; s++) { // seqlen
      ...
      for (int j = 0; j < vocabSize; j++) {
        if (j == (int)tokenized[i + s + 1])
          mTruth(i, j + s * vocabSize) = 1.0f;
        else
          mTruth(i, j + s * vocabSize) = 0.0f;
      }
    }
  }
  ...
  while (1) {
    ...
    cout << "  Loss: " << netFit.get_current_train_loss() << " "
         << netFit.get_current_train_accuracy() << " "
         << netFit.get_current_validation_loss() << " "
         << netFit.get_current_validation_accuracy() << " " << endl;
    if (vocabSize < 10) {
      for (int i = 0; i < vocabSize; i++)
        for (int j = 0; j < vocabSize; j++) {
          ...
          cout << vocabulary[i] << " " << vocabulary[j] << " -> "
               << vocabulary[sampleNextToken(mOut)]; //,vocabSize
          for (int kk = 0; kk < vocabSize; kk++) {
